...
The audio engine is implemented as a pure Web Audio API DSP graph in [`src/audio/AudioEngine.ts`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/audio/AudioEngine.ts).
...
### Unlocking Workflow
1. At startup, [`AudioEngine`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/audio/AudioEngine.ts#L9) initializes the `AudioContext` in a `suspended` state.
2. An overlay banner (`#boot-hint`) displays: `CLICK ANYWHERE TO UNLOCK AUDIO & CAMERA`.
3. When clicked, [`unlock()`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/audio/AudioEngine.ts#L24) executes:
...
For unassisted exhibition deployment where no mouse or keyboard is available, the launcher script ([`scripts/kiosk.sh`](file:///Users/mclovin/Documents/pabellon/v-feed-06/scripts/kiosk.sh)) boots Chromium with the flag:
```bash
...
Rather than playing an MP3 loop of static (which loops noticeably and wastes RAM), [`AudioEngine`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/audio/AudioEngine.ts#L9) procedurally constructs a 2-second looped `AudioBuffer`:
...
- **Frequency**: Configured via [`CRT_FLYBACK_HZ = 15734.26`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/core/constants.ts#L30).
...
Every frame, the audio parameters reactively synchronize with the vision telemetry stored in [`useAppStore`](file:///Users/mclovin/Documents/pabellon/v-feed-06/src/core/StateManager.ts#L452):
...
*Next Step: Explore [06. Video Pipeline & Server Architecture](file:///Users/mclovin/Documents/pabellon/v-feed-06/docs/architecture/06-video-pipeline-and-server.md) to understand how video content is ingested, cached, and served.*
